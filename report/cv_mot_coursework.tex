...
\section{真实场景评估结果}
为进一步验证系统在真实数据上的性能，本项目整合原始实习项目中的实际监控场景数据进行了评估。测试场景为某密集路口，包含2474帧视频、117个独立行人ID、约33846条标注数据。

评估结果如表\ref{tab:real}所示。系统在该真实场景中达到IDF1 73.26\%，IDP 94.01\%，IDR 60.01\%，MOTA 55.49\%，发生ID Switches 236次。该结果表明：检测精确率较高（IDP 94\%），但召回率相对较低（IDR 60\%），说明存在一定比例的漏检；IDF1达到73\%表明整体身份匹配能力良好；236次身份切换主要发生在目标密集交叉和严重遮挡区域。
...
\begin{tabular}{lccccc}
\toprule
指标 & IDF1(\%) & IDP(\%) & IDR(\%) & MOTA(\%) & IDs$\downarrow$ \\
\midrule
真实场景 & 73.26 & 94.01 & 60.01 & 55.49 & 236 \\
\bottomrule
\end{tabular}
\end{table}

评估过程中的详细日志（TP: 20310, FP: 1293, FN: 13536）已保存至 \texttt{original\_project/run\_logs/} 目录，可供复查。
